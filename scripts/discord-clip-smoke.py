...
Loads:
  ...
  - $DISCORD_PY_SELF_ROOT/.env for DISCORD_USER_TOKEN, DISCORD_TEST_GUILD_ID, DISCORD_TEST_CHANNEL_ID
...
async def main() -> int:
    ...
    item_id = os.environ.get("JELLYBOT_SMOKE_ITEM_ID", "").strip()

    if not token or not app_id or not channel_id or not item_id:
        print("Missing DISCORD_USER_TOKEN, DISCORD_CLIENT_ID, DISCORD_TEST_CHANNEL_ID, or JELLYBOT_SMOKE_ITEM_ID")
        return 2
    ...
    async def on_ready() -> None:
        ...
        print("Invoking /clip with configured item id (bypasses autocomplete UI)")
        try:
