...
| **Also tagged** | `:main`, `:sha-<commit>` on each Ship main run; semver tags when `feat`/`fix` releases are cut |

Production uses Watchtower (`watchtower-minutely`, label-enabled) to recreate the container when `:latest` digest changes. See **[Issue → deployment](docs/ISSUE_TO_DEPLOYMENT.md)** for the full pipeline. The bot posts major/minor release embeds to Discord **once per restart** via `on_ready`.
